...
class Collection(models.Model):
    ...
    def clean(self):
        ...
        if (
            self.visibility == 1 and self.pk is not None
        ):  # For existing collections being changed to private
            ...
            for item in [ci.item for ci in collection_items]:
                ...
                if (
                    CollectionItems.objects.filter(item=item)
                    .exclude(collection=self)
                    .exists()
                ):
                    raise ValidationError(
                        f"Item '{item.title}' is in multiple collections and cannot be in a private collection."
                    )
...
class CollectionItems(models.Model):
    ...
    def clean(self):
        ...
        if self.collection.visibility == 1:  # Private
            ...
            other_collections = CollectionItems.objects.filter(item=self.item).exclude(
                collection=self.collection
            )
            if other_collections.exists():
                raise ValidationError(
                    "This item already exists in another collection and cannot be added to a private collection."
                )

        # Check if this item is already in a private collection
        private_collections = CollectionItems.objects.filter(
            item=self.item, collection__visibility=1  # Private
        ).exclude(collection=self.collection)

        if private_collections.exists():
            raise ValidationError(
                "This item is already in a private collection and cannot be added to another collection."
            )
