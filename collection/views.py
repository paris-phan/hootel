...
def create_collection(request):
    if request.method == 'POST':
        ...
        messages.success(request, f'Collection "{title}" created successfully. You can now add items to it.')
        ...
        if item_ids:
                ...
                if int(item_id) not in existing_item_ids:
                    ...
                    try:
                        ...
                        if collection.visibility == 1:
                            ...
                            existing_collections = [entry.collection.title for entry in existing_entries]
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    ...
                    'message': success_message if (items_added > 0 or items_moved > 0) else "Failed to add items",
                    'items_added': items_added,
                    'items_moved': items_moved,
                    'errors': [err[1] for err in failed_items] if failed_items else []
                })
        else:
