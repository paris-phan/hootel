...
def create_access_request(request):
    ...
    try:
        ...
        access_request = AccessRequest.objects.create(
            user=request.user, collection=collection, reason=reason
        )
...
def approve_access_request(request, request_id):
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Access request approved successfully"}
        )
    except Exception as e:
        return JsonResponse({"success": False, "message": str(e)}, status=500)
...
def deny_access_request(request, request_id):
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Access request denied successfully"}
        )
    except Exception as e:
        return JsonResponse({"success": False, "message": str(e)}, status=500)
